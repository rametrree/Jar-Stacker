		...
		UseEntityCallback.EVENT.register((Player player, Level level, InteractionHand hand, Entity entity, EntityHitResult hitResult) -> {
			...
			if (decision.requiresExtraction()) {
				...
				if (count > 1) {
					...
					if (mob instanceof VariantHolder<?> vSrc && extracted instanceof VariantHolder<?> vDst) {
					...
					if (mob instanceof net.minecraft.world.entity.Saddleable sSrc && extracted instanceof net.minecraft.world.entity.Saddleable sDst) {
						if (sSrc.isSaddled()) {
							sDst.equipSaddle(ItemStack.EMPTY, null);
						}
					}
